...
import { Divider,notification,Icon,Progress,Steps, Button, message } from 'antd';
...
class RightSidebarComponent extends React.Component<IProps, IState> {
    ...
    AlertMessage(msg: any, type: any) {
      ...
      validateBadgeInfoTab=()=>{
        let flagError=false;
        if(this.state.jobExperience.length<2)//as 0 is default value 
        {flagError=true;}
        if(flagError)
        {
        notification.open({
            ...
            description:
            'Please update your job experience to proceed in next page.',
            duration: 0,
    ...
    render() {
      return (
        ...
        <Button style={{ marginLeft: 8 }} onClick={() => this.props.prevStep()}>
          ...
          <Button type="primary" onClick={() => this.validateBadgeInfoTab()}>
              Next
              <Icon type="right" />
          </Button>
          
        </div>
        <Divider>Achevement Details</Divider>

        <div className="col-lg-3 col-md-3 sm-hidden xs-hidden"></div>
        ...
        <div className="col-lg- col-md-3 sm-hidden xs-hidden"></div>
